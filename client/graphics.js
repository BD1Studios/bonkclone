    ...
    renderMap(gst){
        ...
        for(let i = 0; i != gst.physics.bodies.length; i++){
            ...
            for(let i = 0; i != body.fx.length; i += 0.5){
                let fixture = gst.physics.fixtures[body.fx[Math.floor(i)]]
                let shape = gst.physics.shapes[fixture.sh];
                let ppm = gst.physics.ppm;
                let pointArray = [];
                let isShadow = Math.floor(i) == i;
                let shadowDist = 2;
                ...
                switch(shape.type){
                    case "bx":
                        let width = shape.w / 2 * ppm * this.scaleRatio;
                        let height = shape.h / 2 * ppm * this.scaleRatio;
                        pointArray = [];
                        ...
                        for(let i = 0; i != pointArray.length; i++){
                            ...
                            if (isShadow) {pointArray[i].x += shadowDist; pointArray[i].y += shadowDist;}
                        }
                    ...
                    case "ci":
                        ...
                        if (isShadow) {pos.x += 2; pos.y += shadowDist;}
...
class discGraphic {
    ...
    render(gst){
        let disc = gst.discs[this.discID];
        let ppm = gst.physics.ppm;
