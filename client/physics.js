...
class bonkPhysics {
    ...
    step(seconds, gst, inputs) {
        ...
        for (let i = 0; i != gst.physics.shapes.length; i++) {
            let shapeObj;
            let pointArray;
            let shape = gst.physics.shapes[i];
            switch (shape.type) {
                case "bx":
                    shapeObj = new b2PolygonShape;
                    let width = shape.w / 2;
                    let height = shape.h / 2;
                    pointArray = [];
        ...
        for (let i = 0; i != gst.physics.bodies.length; i++) {
            ...
            let bodyObj = this.world.CreateBody(bodyDef);
            for (let i2 = 0; i2 != body.fx.length; i2++) {
                let fixture = gst.physics.fixtures[body.fx[i2]];
        ...
        for (let i = 0; i != gst.physics.joints.length; i++) {
            let joint = gst.physics.joints[i];
            let bodyA = joint.ba == -1 ? this.world.GetGroundBody() : bodies[joint.ba];
            let bodyB = joint.bb == -1 ? this.world.GetGroundBody() : bodies[joint.bb];
            let jointObj;
            switch (joint.type) {
                case "lpj":
                    let ang = joint.pa - bodyA.GetAngle();
                    let cosAng = bonkMath.fixedTrig.cos(ang);
                    let sinAng = bonkMath.fixedTrig.sin(ang);
        ...
        for (let i = 0; i != gst.discs.length; i++) {
            ...
            let disc = gst.discs[i];

            let discShapeDef = new b2CircleShape(1);
            let discFixtureDef = new b2FixtureDef();
            let discBodyDef = new b2BodyDef();
            ...
            let discBody = this.world.CreateBody(discBodyDef);
            discBody.CreateFixture(discFixtureDef);

            let moveVector = new b2Vec2(0, 0);
            let moveAcc = 12;
            ...
            if (inputs[i].up && Math.abs(disc.yv) < 4) {
                let rayPointA = new b2Vec2(disc.x, disc.y);
                let rayPointB = new b2Vec2(disc.x, disc.y + 1.15);
                this.world.RayCast(() => {
